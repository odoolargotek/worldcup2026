// news.js — Noticias del Mundial 2026 via Cloud Function
const USE_CF = true;
...
function renderFallback() {
  ...
  return `
    <div style="background:linear-gradient(135deg,rgba(74,175,212,0.06),rgba(245,158,11,0.04));border:1px solid rgba(74,175,212,0.2);border-radius:14px;padding:20px;margin-bottom:16px;text-align:center">
      ...
      <div style="font-weight:700;font-size:0.95rem;margin-bottom:4px">No se pudieron cargar las noticias</div>
      <div style="font-size:12px;color:var(--text-muted)">Revisa tu conexión e intenta de nuevo</div>
    </div>
...
export async function renderNews() {
  ...
  async function load() {
    ...
    if (!listEl) return;
    listEl.innerHTML = items.length ? items.map(renderCard).join('') : renderFallback();
    const now = new Date().toLocaleTimeString('es', { hour:'2-digit', minute:'2-digit' });
    if (updateEl) updateEl.textContent = items.length ? `${items.length} noticias · ${now}` : 'Sin conexión';
