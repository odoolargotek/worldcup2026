// matches.js — Partidos agrupados por FECHA para facilitar pronósticos diarios
...
function deadlineBadge(kickoff, isFinal, hasScore, hasMyPred) {
  ...
  if (diffMs <= 0 && hasScore) {
    return '<span style="background:rgba(239,68,68,0.25);color:#fca5a5;font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;white-space:nowrap">🔴 EN VIVO</span>';
  }
  if (diffMs <= 0) {
  ...
  if (diffMins <= 60) {
    ...
    return '<span style="background:rgba(239,68,68,0.2);color:#fca5a5;font-size:10px;font-weight:700;padding:2px 8px;border-radius:20px;white-space:nowrap">⏰ ¡' + mins + ' min!</span>';
  }
  if (diffHrs <= 3) {
...
function renderMatches(snap) {
  ...
  snap.forEach(d => {
    const m       = d.data();
    const kickoff = m.kickoff && m.kickoff.toDate ? m.kickoff.toDate() : new Date(m.kickoff);
    const key     = toLocalDateKey(kickoff);
    if (!byDate[key]) byDate[key] = [];
    byDate[key].push(Object.assign({ id: d.id }, m, { _kickoff: kickoff }));
  });
      ...
      '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">' +
        ...
        '<div style="display:flex;align-items:center;gap:4px;white-space:nowrap">' +
          '<span style="color:' + lbl.color + ';font-size:0.85rem;font-weight:800;text-transform:capitalize;letter-spacing:1px">' +
            lbl.emoji + ' ' + lbl.text +
          '</span>' +
          '<span style="color:var(--text-muted);font-size:0.75rem;font-weight:500"> · ' + matches.length + ' partido' + (matches.length > 1 ? 's' : '') + '</span>' +
          alertChip +
        '</div>' +
        '<div style="flex:1;height:1px;background:var(--border)"></div>' +
      '</div>';
